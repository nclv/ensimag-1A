...
QUEST, PLUS, MINUS, MULT, DIV, INT, VAR, END, OPAR, CPAR = TOKENS
TOKEN_PREFIX = ('?', '+', '-', '*', '/', '', '#', '', '(', ')')
SPACES = {' ', '\n', '\t'}
...
def mk_switch():
    d = {'': (END, parse_END)}
    PARSERS = (parse_DEFAULT, parse_DEFAULT, parse_DEFAULT, parse_DEFAULT,
               parse_DEFAULT, parse_INT, parse_VAR, parse_END, parse_DEFAULT,
               parse_DEFAULT
               )
    for indice_token, token in enumerate(TOKEN_PREFIX):
        if indice_token == 5:
            for digit in range(10):
                d[str(digit)] = (indice_token, PARSERS[indice_token])
        else:
            d[str(token)] = (indice_token, PARSERS[indice_token])
